...
const columns: ColumnDef<OrangMasuk>[] = [
...
export default function OrangMasukPage() {
  const { isMaster, businessUnitFilter, loading: authLoading } = useAuth();
  const [selected, setSelected] = useState<OrangMasuk | null>(null);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Orang Masuk</h1>
        <p className="text-muted-foreground text-sm">
          Catatan akses masuk personel
        </p>
      </div>
      <DataTable
        columns={columns}
